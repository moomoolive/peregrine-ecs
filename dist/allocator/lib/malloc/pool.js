...
const align_1 = require("@thi.ng/binary/align");
const assert_1 = require("@thi.ng/errors/assert");
...
class MemPool {
    constructor(opts = {}) {
        ...
        this.start = opts.start != null ? (0, align_1.align)(Math.max(opts.start, 0), 4) : 0;
        ...
        if (!opts.skipInitialization) {
            ...
            (0, assert_1.assert)(_align >= 8, `invalid alignment: ${_align}, must be a pow2 and >= 8`);
            const top = this.initialTop(_align);
            const resolvedEnd = opts.end != null
                ? Math.min(opts.end, this.buf.byteLength)
                : this.buf.byteLength;
            if (top >= resolvedEnd) {
                (0, illegal_arguments_1.illegalArgs)(`insufficient address range (0x${this.start.toString(16)} - 0x${resolvedEnd.toString(16)})`);
            }
            this.align = _align;
    ...
    mallocAs(type, num) {
        const addr = this.malloc(num * typedarray_1.SIZEOF[type]);
        return addr ? (0, typedarray_1.typedArray)(type, this.buf, addr, num) : undefined;
    }
    calloc(bytes, fill = 0) {
    ...
    malloc(bytes) {
        ...
        const paddedSize = (0, align_1.align)(bytes + SIZEOF_MEM_BLOCK, this.align);
        const end = this.end;
        let top = this.top;
    ...
    set minSplit(x) {
        (0, assert_1.assert)(x > SIZEOF_MEM_BLOCK, `illegal min split threshold: ${x}, require at least ${SIZEOF_MEM_BLOCK + 1}`);
    ...
    initialTop(_align = this.align) {
        return ((0, align_1.align)(this.start + SIZEOF_STATE + SIZEOF_MEM_BLOCK, _align) -
            SIZEOF_MEM_BLOCK);
    }
    /**
